...
class Flux2KleinPipeline(DiffusionPipeline, Flux2LoraLoaderMixin):
    ...
    def _unpack_latents_with_ids(
        x: torch.Tensor, x_ids: torch.Tensor, height: Optional[int] = None, width: Optional[int] = None
    ) -> list[torch.Tensor]:
        """
    ...
    def _encode_vae_image(self, image: torch.Tensor, generator: torch.Generator):
        ...
        vae_image = image.to(self.vae.dtype)

        image_latents = retrieve_latents(self.vae.encode(vae_image), generator=generator, sample_mode="argmax")
        ...
        return image_latents.to(orig_dtype)
    ...
    def __call__(
        self,
        image: Union[list[PIL.Image.Image], PIL.Image.Image, None] = None,
